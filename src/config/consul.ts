...
class ConsulInstance {
  static instance: ConsulInstance;
  static getInstance(service: string, uri: string) {
    if (!ConsulInstance.instance) {
      ConsulInstance.instance = new ConsulInstance(service, uri);
    }
    return ConsulInstance.instance;
  }
  ...
  private constructor(service: string, uri: string) {
    const consulHost =
    ...
    let healthCheckUrl = "http://localhost:3000/mail-service/healthcheck";
    if (process.env.NODE_ENV === "production") {
      healthCheckUrl = `http://${address}:3000/mail-servie/healthcheck`;
    }
